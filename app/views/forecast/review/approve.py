...
class ForecastApprove(APIView):
    ...
    def put(self, request, forecast_type):
        try:
            ...
            if forecastType == 0:
                logger.error(f"DPAI Service: {forecast_type}ForecastReview-PUT: Invalid ForecastType: tenant_id: {self.request.query_params.get('tenant_id')} | bu_id: {self.request.query_params.get('bu_id')} | snop_id: {self.request.query_params.get('snop_id')}")
            ...
            if check_tenant_buid_snop_id is False:
                logger.error(f"DPAI Service: {forecast_type}ForecastReview-PUT: UNAUTHORIZED: tenant_id: {self.request.query_params.get('tenant_id')} | bu_id: {self.request.query_params.get('bu_id')} | snop_id: {self.request.query_params.get('snop_id')}")
            ...
            if request.data and 'forecastHeaders' in request.data and request.data['forecastHeaders']:
                result = ForecastReviewService.approve(self, self.request.query_params.get('tenant_id'), self.request.query_params.get('bu_id'), self.request.query_params.get('snop_id'), request.data, str.replace(str(request.META.get('HTTP_AUTHORIZATION')), 'Bearer ', ''), ForecastType(forecastType).name)
            else:
                logger.error(f"DPAI Service: {forecast_type}ForecastReview-PUT: Forecast review approved rejected body: {request.data}")
            ...
            logger.error(f"DPAI Service: {forecast_type}ForecastReview-PUT: Forecast review approved rejected with error: {e}")
            return base_util.createFailureResponse(self, ResponseCodes(5).name, 'Forecast review rejected!', status.HTTP_400_BAD_REQUEST)